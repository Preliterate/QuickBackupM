...
def wait_for_cancel_text(server, info, slot):
    for countdown in range(9, -1, -1):
        print_message(server, info, command_run(
            '还有{}秒，将§c回档§r为槽位§6{}§r，{}'.format(countdown, slot, format_slot_info(slot_number=slot)),
            '点击终止回档！',
            '{} abort'.format(Prefix)
        ), tell=False)
        for i in range(10):
...
def wait_for_cancel_with_progress_bar(server_, info, slot):
    global progress_bar_instance
    print_message(server_, info, command_run(
        '即将§c回档§r为槽位§6{}§r，{}'.format(slot, format_slot_info(slot_number=slot)),
        '点击终止回档！',
        '{} abort'.format(Prefix)
    ), tell=False)
    ...
    for countdown in range(9, -1, -1):
        ...
        for i in range(10, 0, -1):
            ...
            global abort_restore
            if abort_restore:
                print_message(server_, info, '§c回档§r被中断！', tell=False)
                progress_bar.delete()
                return False
    progress_bar.delete()
    return True
...
def on_load(server, old):
    ...
    global creating_backup, restoring_backup, progress_bar_instance, wait_for_cancel
    ...
    if progress_bar_instance is not None:
        print(progress_bar_instance)
        server.logger.info('检测到您已安装ProgressBar，自动开启倒计时进度条。')
        wait_for_cancel = wait_for_cancel_with_progress_bar
    else:
        wait_for_cancel = wait_for_cancel_text
